...
# Base builder image with essential tools
FROM condaforge/mambaforge:24.1.0-0 AS builder

# Configure conda for better caching and reliability
RUN mamba config --system --set remote_max_retries 10 \
    --set remote_backoff_factor 2 \
    --set channel_priority strict \
    && mamba clean --all -f -y

# Create retry script for conda operations
RUN printf '#!/bin/sh\nfor i in 1 2 3 4 5; do "$@" && exit 0; sleep 15; done; exit 1' > /usr/bin/retry \
    && chmod +x /usr/bin/retry

# Generic build template for conda packages
FROM builder AS package_builder
ARG MAMBA_DOCKERFILE_ACTIVATE=1
WORKDIR /recipes

# Common build command with aggressive cleanup
COPY recipes ./recipes

# Build all packages in sequence with layer optimization
